...
export interface CreatePassportInput {
  ...
  granularity: Granularity; // 'ProductClass' | 'Batch' | 'Item'
  ...
  // Required if granularity = 'Batch'
  batchNumber?: string;
  
  // Required if granularity = 'Item'
  serialNumber?: string;
...
export interface CreatePassportResult {
  ...
  txHash: string;
  blockNumber: number;
  granularity: Granularity; // Added for v0.2
  subjectIdHash: string; // SHA-256 of canonical subject ID
}
